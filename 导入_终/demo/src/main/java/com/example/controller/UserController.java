...
public class UserController {
    ...
    public void downloadAllClassmate(HttpServletResponse response) throws IOException {
        HSSFWorkbook workbook = new HSSFWorkbook();//create HSSFWorkbook
        HSSFSheet sheet = workbook.createSheet("信息表"); //create sheet

        List<User> classmateList = userService.userinfor();

        String fileName = "userinf" + ".xls";//file name
        //new row
        int rowNum = 1;
    ...
    public String uploadFile(MultipartFile file) {
        try {
            List<User> users = new ExcelUtil().readUsers(file.getInputStream());
            for (User user : users) {
                userService.save(user);
            }
            return "ok";
